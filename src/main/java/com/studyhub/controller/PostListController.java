...
public class PostListController {
    ...
    @Autowired
    PostListController(PostListService postListService, UserProfileService userProfileService, FileUploadService fileUploadService) {
    ...
    public String getPostList(@AuthenticationPrincipal StudyHubUserDetails userDetails,
        ...
        Long scopedAuthorId = user.getRole() == UserRole.MARKETING ? user.getId() : null;

        org.springframework.data.domain.Sort sort = direction.equalsIgnoreCase("asc") ?
                org.springframework.data.domain.Sort.by(sortBy).ascending() :
                org.springframework.data.domain.Sort.by(sortBy).descending();

        List<Post> posts = postListService.filterPosts(searchTitle, searchCategory, searchAuthor, scopedAuthorId, sort);
    ...
    public String saveNewPost(@Valid @ModelAttribute("newPostDTO") NewPostDTO newPostDTO,
        ...
        if (file != null && !file.isEmpty()) {
            try {
                post.setThumbnail(fileUploadService.uploadImage(file));
    ...
    public String switchPostStatus(@RequestParam(name = "id") Long id,
        ...
        try {
            ...
            postListService.switchPostStatus(id, status, user.getId(), user.getRole());
        } catch (Exception e) {
    ...
    public String saveUpdatePost(@Valid @ModelAttribute("newPostDTO") NewPostDTO newPostDTO,
        ...
        if (file != null && !file.isEmpty()) {
            try {
                fileUploadService.delete(post.getThumbnail());
                post.setThumbnail(fileUploadService.uploadImage(file));
            } catch (IllegalArgumentException e) {
                List<Setting> categories = postListService.getAllCategories();
                model.addAttribute("user", user);
